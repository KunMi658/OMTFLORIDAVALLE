...
const Sync = {
  ...
  paths: {
    ...
    kicked: (key) => ref(db, `${SESSION_KEY}/students/${key}/kicked`)
  },
  ...
  async resetSession() {
    await set(this.paths.session(), null);
    await set(ref(db, `${SESSION_KEY}/students`), null);
    await set(ref(db, `${SESSION_KEY}/activity`), null);
    await set(ref(db, `${SESSION_KEY}/responses`), null);
    await set(this.paths.hotSeat(), null);
  ...
  async kickStudent(key) {
    // Set kicked flag on their student node and mark offline
    await update(this.paths.student(key), { kicked: true, online: false });
