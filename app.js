...
// Modulos: App (splash/nav) | ProfControl | StudentLive | ProjectorView
// ============================================
...
const App = {
    ...
    async studentJoinClass() {
          ...
          try {
                  ...
                  document.getElementById('student-login-error').textContent = 'Error de conexion. Verifica tu internet.';
    ...
    professorEnter() {
          ...
          if (pass !== '6588242') {
                  document.getElementById('professor-login-error').textContent = 'Codigo incorrecto';
                  return;
          }
          ProfControl.init();
...
const ProjectorView = {
    ...
    handleHotSeatProjector(hotSeat) {
      ...
      document.querySelector('.proj-activity-badge').textContent = 'SILLA CALIENTE';
          ...
          document.getElementById('proj-timer-num').textContent = 'Inf';
    ...
    handleSession(session) {
          if (!session || !session.active) {
                  this.showWaiting('El profesor esta preparando la sesion...');
                  return;
          }
      ...
      if (slide.type === 'info') {
            ...
            container.innerHTML = `<div class="proj-exercise-preview" style="opacity:0.4">
                    <div class="proj-exercise-badge">CLASE EN CURSO</div>
                            <div class="proj-exercise-hint">El profesor continua la explicacion...<br>La actividad se lanzara cuando este listo.</div>
                                  </div>`;
      }
    },
    ...
    handleActivity(activity) {
      ...
      const optionsEl = document.getElementById('proj-activity-options');
          if (activity.type === 'multiple' && activity.options) {
                  ...
                  optionsEl.innerHTML = `<div class="proj-text-indicator">Respuesta abierta - escribela en tu celular</div>`;
          }
    ...
    handleResponses(responses) {
      ...
      container.innerHTML = `<div class="proj-responses-header">Mail ${count} respuesta${count > 1 ? 's' : ''} recibida${count > 1 ? 's' : ''}</div>
            <div class="proj-response-chips">${Object.values(responses).map(r =>
                      `<div class="proj-response-chip ${r.score > 0 ? 'chip-correct' : 'chip-wrong'}">
                                ${r.score > 0 ? 'OK' : 'Wait'} ${r.studentName.split(' ')[0]}
                                        </div>`
                                                                                  ).join('')}</div>`;
    },
      ...
      let correctAnswerHtml = '';
          if (activity.type === 'multiple' && activity.options) {
                  ...
                  correctAnswerHtml = `<div class="res-correct-banner">
                          <span class="res-correct-label">OK Respuesta correcta</span>
    ...
    const barHtml = `
          <div class="res-bars-section">
                  <div class="res-bars-title">Puntaje acumulado de la sesion</div>
                          ...
                          <div class="res-cards-grid" id="res-cards-grid">
                                    ${studentList.map((s, i) => {
                                                  ...
                                                  const stars = '*'.repeat(starCount) + '.'.repeat(5 - starCount);
                                                                  ...
                                                                  let answerDetail = '';
                                                  if (!r) {
                                                                  answerDetail = `<div class="res-card-no-answer">No respondio a tiempo</div>`;
  ...
  updateStudentCount(students) {
    ...
    const waitingEl = document.getElementById('proj-waiting_students');
        if (waitingEl) {
...
const ProfControl = {
    ...
    async init() {
          ...
          document.getElementById('prof-session-info').textContent = 'Sesion activa';
    },
    ...
    async launchHotSeat() {
          ...
          if (list.length === 0) return alert('No hay estudiantes en linea');

      const randomStudent = list[Math.floor(Math.random() * list.length)];
          const slide = this.currentClass.studentSlides[this.currentSlide];
          const question = slide && slide.type === 'exercise' ? slide.question : "Pregunta sorpresa de atencion.";
                  ...
                  `<div class="prof-student-item">
                          <span class="prof-student-dot online"></span>
                                  <span class="prof-student-name">${s.name}</span>
                                          <span class="prof-student-score">Star ${s.score || 0}</span>
                                                </div>`
    ...
    async confirmReset() {
          if (confirm('?Resetear la sesion? Todos los estudiantes seran desconectados.')) {
...
const StudentLive = {
    ...
    init(name, key) {
          ...
          this.key = key;
          this.answered = false;
          document.getElementById('student-name-badge').textContent = `(Student) ${name.split(' ')[0]}`;
          document.getElementById('student-score-badge').textContent = 'Star 0 pts';
      ...
      this.hotSeatActive = true;
          if (hotSeat.studentKey === this.key) {
            ...
            document.getElementById('sl-text-area').style.display = 'block';
                  ...
                  document.getElementById('sl-timer-display').textContent = 'Inf';
    ...
    handleSession(session) {
          if (!session || !session.active) {
                  ...
                  document.getElementById('sl-waiting-msg').textContent = 'Sesion no iniciada...';
                  return;
          }
      ...
      const update = () => {
              ...
              if (remaining <= 0) {
                        ...
                        if (!this.answered) {
                                    ...
                                    document.getElementById('sl-answered-msg').textContent = 'TIME Tiempo agotado';
                        }
              }
      };
    ...
    async submitMultiple(optionIdx) {
      ...
      document.getElementById('sl-answered-msg').style.display = 'block';
          document.getElementById('sl-answered-msg').textContent =
                  score > 0 ? `OK !Correcto! +${score} puntos` : 'Wrong Incorrecto - !a repasar!';
      ...
      const snap = await Sync.getStudents();
          if (snap[this.key]) {
                  document.getElementById('student-score-badge').textContent = `Star ${snap[this.key].score} pts`;
          }
    },
    ...
    async submitText() {
      ...
      document.getElementById('sl-text-area').style.display = 'none';
          ...
          document.getElementById('sl-answered-msg').textContent = this.hotSeatActive
            ? 'OK Respuesta enviada. El profesor la leera en voz alta.'
                  : 'OK Respuesta enviada. El profesor la revisara.';
    ...
    async showResultsState() {
      ...
      document.getElementById('student-score-badge').textContent = `Star ${score} pts`;
          document.getElementById('sl-results-content').innerHTML = `
                <div class="sl-result-icon">${score > 0 ? 'Star' : 'Book'}</div>
                      <h3 class="sl-result-title">${score > 0 ? '!Bien hecho!' : 'Sigue practicando'}</h3>
                            <div class="sl-result-score">${score} puntos acumulados</div>
                                  <p class="sl-result-msg">${score > 0
                                                                     ...
                                                                     : 'No te preocupes, repasa el tema y lo dominaras.'}</p>
                                                                           <div class="sl-waiting-next">Wait Esperando siguiente actividad...</div>
                                                                               `;
    },
                                                                            ...
                                                                            <div class="reconnect-box">
                                                                                          <p>Clase en curso detectada.</p>
                                                                                                        <p><strong>?Deseas continuar como ${savedName.split(' ')[0]}?</strong></p>
                                                                                                                      <button class="btn btn-primary" style="margin-bottom:8px;" onclick="App.reconnect('${savedName}', '${savedKey}')">Play Continuar Clase</button>
